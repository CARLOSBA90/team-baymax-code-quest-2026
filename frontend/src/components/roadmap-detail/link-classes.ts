...
// `components/roadmap-detail/` (no se exporta desde el barrel). Sin altura ni padding: cada
// consumidor añade los suyos, porque dos `h-*` en la misma clase compiten por orden de CSS.

const LINK_SHAPE =
  "flex w-fit items-center justify-center gap-2 rounded-xl border font-body outline-none transition-[filter,background-color,border-color,box-shadow] duration-150 ease-out focus-visible:border-accent-hover focus-visible:shadow-ring-focus";
